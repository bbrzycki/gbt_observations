...
# Adapted from Steve Croft's observability script
import numpy as np
import scipy as sp
...
parser.add_argument('-radec',help="Use RA/Dec",action="store_true", default=False)
parser.add_argument('-gal',help="Use galactic coordinates",action="store_true", default=False)
args = parser.parse_args()

#target = SkyCoord.from_name('LHS1140')
if args.gal:
	target = SkyCoord(args.source, unit=(u.deg, u.deg), frame='galactic')
	target = target.icrs
elif args.radec:
	target = SkyCoord(args.source, unit=(u.hourangle, u.deg))
else:
...
gal_coord = target.galactic
print("Target l=",gal_coord.l," b=",gal_coord.b)
print(gal_coord.l.value,gal_coord.b.value)
...
if args.gal:
    save_fn = f'observability_gal_{args.source}_{tstart}.pdf'
elif args.radec:
    save_fn = f'observability_radec_{args.source}_{tstart}.pdf'
else:
	save_fn = f'observability_{args.source}_{tstart}.pdf'
